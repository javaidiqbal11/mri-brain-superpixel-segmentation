...
% Function call:
%   [x] = haralickTextureFeatures(coOcMat);
% calculates all  Haralick Features:
% 	[x] = haralickTextureFeatures(coOcMat, xFeatures); 
%     
%	Calculates the Haralick Features specified by xFeatures, the rest will be return as 0.
...
%               x(15) Standard deviation [checked]
%               x(16) Average [checked ]
%
% Example:
%   ---------      
...
% 
%	%calculate feature 4 (Variance), 5 (Inverse Difference Moment) and 6 (Sum Average)
%   xFeatures = 4:6;
...
%   %Get only the features you want
 %  x = x( xFeatures );    
%  
%   %Get all features mentiond in xFeatures 
%   xFeatures = 1:16;
%   x = haralickTextureFeatures(glcm, 1:16);
%
%
%%
% check input
...
if sum(xFeatures == 2) == 1 || ...      % Contrast
        ...
        sum(xFeatures == 15) == 1 ||... %standard devition 
        sum(xFeatures == 16) == 1       %Average 
...
for f = xFeatures
    switch f
        ...
        case (15) %Standard deviation 
            x(15) = sqrt(sum( (row(:)-mean(coOcMat(:))).^2 .*coOcMat(:) ));
            %x(15)=sqrt(res);
        case (16) %Average 
            x(16) = mean((2:(2*sizecoOcMat(1)))' .* p_XplusY) ;
    end
